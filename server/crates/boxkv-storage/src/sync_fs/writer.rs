//! 同步 WritableFileWriter

use crate::error::StorageError;
use crate::sync_fs::traits::WritableFile;
use bytes::BytesMut;
use std::cmp;
...
impl WritableFileWriter {
    /// 创建新的 WritableFileWriter
    pub fn new(file: Box<dyn WritableFile>, max_buffer_size: Option<usize>) -> Self {
    ...
    /// 追加数据到文件
    pub fn append(&mut self, data: &[u8]) -> Result<(), StorageError> {
    ...
    /// 内部 flush 实现
    fn flush_internal(&mut self) -> Result<(), StorageError> {
        if self.buffer.is_empty() {
            return Ok(());
        }
